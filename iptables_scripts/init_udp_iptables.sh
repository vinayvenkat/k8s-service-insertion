...
function usage() {
  ...
  echo ''
  echo '  -p: Specify the envoy port to which redirect all UDP traffic (default $ENVOY_PORT = 15001)'
...
iptables -t nat -D PREROUTING -p udp -j ISTIO_INBOUND 2>/dev/null
iptables -t nat -D OUTPUT -p tcp -j ISTIO_OUTPUT 2>/dev/null
iptables -t nat -D OUTPUT -p udp -j ISTIO_OUTPUT 2>/dev/null
...
iptables -t nat -A ISTIO_REDIRECT -p udp -j REDIRECT --to-port ${PROXY_PORT}
...
if [ -n "${INBOUND_PORTS_INCLUDE}" ]; then
  ...
  iptables -t nat -A PREROUTING -p udp -j ISTIO_INBOUND

  if [ "${INBOUND_PORTS_INCLUDE}" == "*" ]; then
    # Makes sure SSH is not redirected
    iptables -t nat -A ISTIO_INBOUND -p udp --dport 22 -j RETURN
    # Apply any user-specified port exclusions.
    if [ -n "${INBOUND_PORTS_EXCLUDE}" ]; then
      for port in ${INBOUND_PORTS_EXCLUDE}; do
        iptables -t nat -A ISTIO_INBOUND -p udp --dport ${port} -j RETURN
      done
    fi
    # Redirect remaining inbound traffic to Envoy.
    iptables -t nat -A ISTIO_INBOUND -p udp -j ISTIO_REDIRECT
  else
    # User has specified a non-empty list of ports to be redirected to Envoy.
    for port in ${INBOUND_PORTS_INCLUDE}; do
      iptables -t nat -A ISTIO_INBOUND -p udp --dport ${port} -j ISTIO_REDIRECT
    done
  fi
fi
...
# Jump to the ISTIO_OUTPUT chain from OUTPUT chain for all udp traffic.
